  ...
  const fetchData = async () => {
    try {
      ...
      if (statsRes) {
        ...
        let successCount = 0;
        let errorCount = 0;
        const statusMap: Record<string, number> = { '2xx Success': 0, '4xx Client': 0, '5xx Server': 0 };
        const routes: any[] = [];
        ...
        setStats({
          ...
          latencySeries: allLogs.slice(0, 10).reverse().map(l => ({
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={fetchData} className="p-2 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-colors">
      ...
      {error ? (
        <div className="glass-panel p-12 rounded-3xl border border-rose-500/20 bg-rose-500/[0.02] flex flex-col items-center justify-center text-center">
           ...
           <button onClick={fetchData} className="px-6 py-2 bg-indigo-600 text-white rounded-xl font-bold shadow-lg shadow-indigo-500/20 active:scale-95 transition-all">Verileri Yeniden Yükle</button>
        </div>
      ) : (
        <>
          {/* 2. Top Stats Row */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="glass-panel rounded-3xl h-[450px] border border-white/5 bg-white/[0.01] overflow-hidden flex flex-col">
               <div className="p-6 border-b border-white/5 flex items-center justify-between">
                  <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em]">HIZLI AKIŞ (REACH)</h3>
