...
const ENDPOINTS = [
  { id: 'login', method: 'POST', label: 'POST Login', url: '/api/auth/login', body: '{\n  "email": "admin@ecosystem.com",\n  "password": "admin123"\n}' },
  { id: 'register', method: 'POST', label: 'POST Register', url: '/api/auth/register', body: '{\n  "email": "user@test.com",\n  "password": "password123"\n}' },
  { id: 'products_get', method: 'GET', label: 'GET All Products', url: '/api/products', body: '' },
  { id: 'product_create', method: 'POST', label: 'POST Create Product (admin)', url: '/api/products', body: '{\n  "name": "Gaming PC",\n  "price": 25000,\n  "description": "High end PC",\n  "category": "Elektronik"\n}' },
  { id: 'orders_get', method: 'GET', label: 'GET All Orders', url: '/api/orders', body: '' },
  { id: 'stats', method: 'GET', label: 'GET Admin Traffic Stats', url: '/api/admin/stats', body: '' },
];
    ...
    <div className="flex h-[calc(100vh-180px)] gap-6 animate-in fade-in duration-700">
      ...
      <div className="w-80 glass-panel rounded-2xl flex flex-col overflow-hidden border border-white/5">
        <div className="p-4 border-b border-white/5 bg-white/[0.02]">
           <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">ENDPOINT'LER</p>
        </div>
        <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
          {ENDPOINTS.map(ep => (
            ...
            >
              ...
              <span className="text-xs font-bold truncate group-hover:text-white transition-colors">{ep.label}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 glass-panel rounded-2xl flex flex-col overflow-hidden border border-white/5 p-6 space-y-6">
        <div className="space-y-4">
           <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em]">BİR ENDPOINT SEÇİN</h3>
           ...
           <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="md:col-span-1 space-y-2">
                 <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">METOD</label>
                 <select 
                    value={method} 
              ...
              <div className="md:col-span-3 space-y-2">
                 <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">TOKEN (OTOMATİK DOLDURULUR)</label>
                 <div className="flex items-center gap-2 bg-black/40 border border-white/10 rounded-xl px-4 py-3">
           ...
           {/* Body Editor */}
           <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">REQUEST BODY (JSON)</label>
              <textarea 
                 value={body}
        ...
        <div className="flex-1 flex flex-col space-y-2 min-h-0">
           <div className="flex items-center justify-between">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">YANIT</label>
              {response && (
                <span className={cn(
                  "text-[10px] font-bold px-2 py-0.5 rounded uppercase",
                  response.status < 400 ? "bg-emerald-500/20 text-emerald-400" : "bg-rose-500/20 text-rose-400"
                )}>
