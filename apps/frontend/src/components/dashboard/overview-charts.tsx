...
export function MethodChart({ data = [] }: { data?: any[] }) {
  ...
  return (
    <div className="w-full h-full flex flex-col p-4 overflow-hidden min-h-[180px]">
      <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-4 shrink-0">
        HTTP METOD DAĞILIMI
      </h3>
...
export function StatusDonutChart({ data = [] }: { data?: any[] }) {
  ...
  return (
    <div className="w-full h-full flex flex-col p-6 overflow-hidden min-h-[300px]">
      <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-8 shrink-0">
        DURUM DAĞILIMI
      </h3>
      <div className="flex-1 min-h-0 relative w-full">
        <ResponsiveContainer width="99%" height="100%">
...
function NoData({ label }: { label: string }) {
  return (
    <div className="w-full h-full flex flex-col p-6 items-center justify-center opacity-40 grayscale group hover:grayscale-0 hover:opacity-100 transition-all duration-500">
      <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.3em] mb-4 absolute top-6 left-6">
        {label}
      </h3>
      <div className="flex flex-col items-center gap-2">
        <div className="w-12 h-12 rounded-full border-2 border-dashed border-slate-700 animate-[spin_8s_linear_infinite]" />
        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-4 italic">
          Veri Bekleniyor...
        </p>
      </div>
    </div>
